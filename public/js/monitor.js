...
function pollActiveUsers(){
  ...
  getting.done(function( res ) {
    if (res.status == "ok"){
      ...
      users = JSON.parse(res.allUsers)
      if (res.allUsers.length > maxUser){
        console.log("update: ", maxUser)
        maxUser = res.allUsers.length
        allUsersActivities = JSON.parse(res.usersActivities)
        messageSummary()
        createFullReport()
      }
      displayActiveUsers()
...
function displayActiveUserActivity(){
  ...
  var user = users.find(o => o.userId == id)
  var au = activeUsers.find(o => o.id == id)
  if (user)
...
function loadUserActivities_Monitor(user, activities){
  ...
  var user = allUsersActivities.find(o => o.user_id == id)
...
function messageSummary(){
  ...
  for (var user of allUsersActivities) {
    var activities = JSON.parse(user.activities)
    if (activities.campaigns_logs){
  ...
  var text = `Total delivered messages: ${totalDeliveredMessages} Total failed messages: ${totalFailedMessages}`
  $("#message-count").html(text)
}
...
function createFullReport(){
  ...
  for (var user of users){
    ...
    var item = {
      ...
      //"Size": `${user.maxCampaign} | ${user.minCampaign} | ${user.avgCampaign}`,
  ...
  var table = new Tabulator("#users-list", {
    ...
    columns:[                 //define the table columns
        ...
        //{title:"Max | Min | Avg", field:"Size", hozAlign:"center", width:150, editor:false},
    ...
    rowClick:function(e, row){
      ...
      var user = users.find(o => o.userId == row.getData().UserId)
      var ua = allUsersActivities.find(o => o.user_id == row.getData().UserId)
